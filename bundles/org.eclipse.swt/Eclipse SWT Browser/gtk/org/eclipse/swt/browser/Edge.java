...
static String wstrToString(long psz, boolean free) {	if (psz == 0) return "";
...
/*
 * long[] ppv: This holds the pointer to the COM object, and it will store the
 * result when the method completes successfully. ToIntFunction<IUnknown>
 * callable: This is a functional interface representing the COM method to be
 * called, such as CreateCoreWebView2EnvironmentWithOptions.
 */
static int callAndWait(long[] ppv, ToIntFunction<IUnknown> callable) {
	int[] phr = {COM.S_OK};
	//creates a callback using newCallback(), which is invoked when the COM function completes (either synchronously or asynchronously).
...
private static void processNextOSMessage() {
	...
	display.readAndDispatchWindows();
}
