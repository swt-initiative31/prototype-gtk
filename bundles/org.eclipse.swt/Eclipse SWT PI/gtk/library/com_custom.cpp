...
class SwtWebView2Callback : public ISwtWebView2Callback {
	...
	{
		printf("com_custom.cpp: SwtWebView2Callback Constructor\n");
	}

	~SwtWebView2Callback() {
		JNIEnv *env = nullptr;
		printf("com_custom.cpp: SwtWebView2Callback Destructor\n");
...
public:
	...
	HRESULT Invoke(UINT_PTR arg0, UINT_PTR arg1) override {
		JNIEnv *env = nullptr;
		printf("com_custom.cpp: SwtWebView2Callback Invoke method enter\n");
		if (JVM->GetEnv((void **)&env, JNI_VERSION_1_2) != JNI_OK) {
			printf("com_custom.cpp: SwtWebView2Callback Invoke method E_FAIL\n");
			return E_FAIL;
		}
		if (env->ExceptionCheck()) {
			printf("com_custom.cpp: SwtWebView2Callback Invoke method ExceptionCheck E_FAIL\n");
			return E_FAIL;
		}
		HRESULT result = (HRESULT)env->CallIntMethod(
				object, methodID, (jlong)arg0, (jlong)arg1);
		  // Print JNI environment and method entry status
				    printf("JNI environment obtained: %p\n", env);

				    // Print the jobject and methodID
				    printf("Object address: %p\n", object);
				    printf("MethodID address: %p\n", methodID);

				    // Print the parameters
				    printf("Parameter arg0: %lld\n", (long long)arg0);
				    printf("Parameter arg1: %lld\n", (long long)arg1);
		printf("com_custom.cpp: SwtWebView2Callback Invoke method result  0x%lx\n",result);
		if (env->ExceptionCheck()) {
			return E_FAIL;
		}
		return result;
...
public:
	static IDispatch* Create(JNIEnv *env, jobject host) {
		...
		static INTERFACEDATA iface = {methods, 1};
		 hr = CreateDispTypeInfo(&iface, LOCALE_NEUTRAL, &pTypeInfo);
