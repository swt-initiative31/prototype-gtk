...
import org.eclipse.swt.internal.ole.jna.*;
 ...
 */
public static int IIDFromString(char[] lpsz, GUID lpiid) {

	// Allocate memory for the CLSID char array (including the null terminator)
    Memory clsidMemory = new Memory((lpsz.length + 1) * Native.WCHAR_SIZE);

    // Write the char array to memory
    clsidMemory.write(0, lpsz, 0, lpsz.length);

    // Null-terminate the string
    clsidMemory.setChar(lpsz.length * Native.WCHAR_SIZE, '\0');

	// Call IIDFromString
	int result = Ole32.INSTANCE.IIDFromString(clsidMemory, lpiid);

	// Check result and use the GUID
	if (result == 0) { // S_OK (0) indicates success
//		System.out.println(
//				"GUID: " + lpiid.Data1 + "-" + lpiid.Data2 + "-" + lpiid.Data3 + "-" + new String(lpiid.Data4));
	} else {
